...
if [ -s $CONFIGS/.config/checkapp.txt ]; then
	...
	if [[ ${REPLY} =~ ^[Yy]$ ]]; then
		...
		echo ""
		[[ -f "/opt/GooPlex/scripts/components/$APPLOC.yaml" ]] && echo " ${YELLOW}S${STD} - ${WHITE}$TASK Stable${STD} (recommended)"
		[[ -f "/opt/GooPlex/scripts/components/$APPLOC-beta.yaml" ]] && echo " ${LYELLOW}B${STD} - $TASK Beta"
		[[ -f "/opt/GooPlex/scripts/components/$APPLOC-cf.yaml" ]] && echo " ${LYELLOW}C${STD} - $TASK Stable with CloudFlare"
		[[ -f "/opt/GooPlex/scripts/components/$APPLOC-beta-cf.yaml" ]] && echo " ${LYELLOW}Q${STD} - $TASK Beta with CloudFlare"
		echo "--------------------------------------------------"
		echo ""
